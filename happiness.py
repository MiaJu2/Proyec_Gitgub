...
#import common
#from ib_time_series import TIMESERIES_DATABASE
...
def download_happiness(start_date, records):
    ...
    """
    LOGGER.info("Downloading happiness time series.")
    ...
    data = data_json.json()
    data = [[datetime.datetime.strptime(d['base_date'], "%Y-%m-%d"), d['frequency'],
             float(d['happiness'])] for d in data['objects']]
    # conn = psycopg2.connect("dbname=%s user=fsagols host=localhost" %
    #                        TIMESERIES_DATABASE)
...
if __name__ == "__main__":
    LOGGER = common.define_logger("happiness.log")
